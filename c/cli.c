...
static char doc[] =
    "zstd-seekable-compress — compress files in seekable ZSTD format";
static char args_doc[] = "";

static struct argp_option options[] = {
    {"file", 'f', "FILE", 0, "Input file (default: stdin)", 0},
    {"file decompressed", 'd', "FILE", 0, "Input decompressed file", 0},
    ...
    {"print zst file info", 'i', 0, 0, "Print zst file info", 0},
    {"verbose", 'v', 0, 0, "Enable verbose output", 1},
    {0}};

struct arguments {
	char *input_file;
	char *input_file_decompressed;
	char *output_file;
	int verbose;
	short print_zst_info;
...
int zstd_seekable_print_info(FILE *in_compressed) {
	...
	// seek_table_entry *entries = malloc(footer.Number_Of_Frames *
	// sizeof(seek_table_entry)); fseek(in, -(9 + footer.Number_Of_Frames *
	// sizeof(seek_table_entry)), SEEK_END); fread(entries,
	// sizeof(seek_table_entry), footer.Number_Of_Frames, in);
	...
	// char *ptr_compressed = mmap(NULL, file_stat_compressed.st_size, PROT_READ,
	//                             MAP_PRIVATE, fileno(in_compressed), 0);
	...
	// char *ptr_decompressed = mmap(NULL, file_stat_decompressed.st_size, PROT_READ,
	//                               MAP_PRIVATE, fileno(in_decompressed), 0);
	...
	//     ZSTD_seekTable_getFrameCompressedSize(st, i); printf("Compressed size:
	//     %zu\n", compressed_size); size_t const compressed_start_pos =
	//     ZSTD_seekTable_getFrameCompressedOffset(st, i); printf("Compressed
	//     offset: %zu\n", compressed_start_pos); printf("Compressed printed:\n");
	//     str_print_slice(ptr_compressed, compressed_start_pos, compressed_size);
	//     printf("\n");

	//     size_t const decompressed_size =
	//     ZSTD_seekTable_getFrameDecompressedSize(st, i); printf("Decompressed
	//     size: %zu\n", decompressed_size); size_t const decompressed_start_pos =
	//     ZSTD_seekTable_getFrameDecompressedOffset(st, i); printf("Decompressed
	//     offset: %zu\n", decompressed_start_pos); printf("Decompressed
	//     printed:\n"); str_print_slice(ptr_decompressed, decompressed_start_pos,
	//     decompressed_size); printf("\n");
...
int do_compress(FILE *in, FILE *out, int compression_level,
                unsigned int frame_size) {
	int ret = ERR_OK;
...
int main(int argc, char **argv) {
	...
	if (!args.print_zst_info) {
		...
		case ERR_ZSTD_INIT:
			fprintf(stderr,
			        "Error: Failed to initialize ZSTD seekable compressor.\n");
			return ret;
		...
		if (in == stdin) {
			fprintf(stderr, "Error: stdin cannot be inspected as zst file.\n");
			return 1;
		}

		// FILE *decompressed = NULL;
		// if (strlen(args.input_file_decompressed) == 0) {
		// 	decompressed = fopen(args.input_file_decompressed, "rb");
		// 	if (!in) {
		...
		if (zstd_seekable_print_info(in)) {
